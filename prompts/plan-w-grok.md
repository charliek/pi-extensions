---
description: Generate a gauntlet-schema plan and run a risk-focused Grok/high review
argument-hint: "[--slug SLUG] [--repository NAME] [--model PROVIDER/MODEL] [--thinking LEVEL] [--] <brief>"
---

# /plan-w-grok — plan plus Grok review

Generate and review a plan using canonical `/plan --review grok` semantics. This template is self-contained; do not invoke another slash command.
...
6. Launch `px-plan-risk-reviewer` with default `cursor/grok-4.5`, thinking `high`; explicit `--model`/`--thinking` wins, then composed-workflow override, default, parent fallback. Use `run_in_background: false` unless the user asked otherwise.
...
Arguments: ${@:-"(brief required after optional flags or `--`; Grok review fixed)"}
