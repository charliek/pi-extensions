---
description: Correctness review for scoped changes — report regressions and missing tests; do not fix
argument-hint: "[--staged | --ref REV] [--model PROVIDER/MODEL] [--thinking LEVEL] [--path FILE]... [--focus TEXT]"
---

# /code-review — correctness review (parent orchestration)

Run a read-only correctness review on the active Git repository. **Do not fix findings** unless the user explicitly asks afterward.
...
- Report concrete regressions and missing tests only — no style or speculative nitpicks.
...
Summarize the verdict and findings in a compact table: Severity, Location, Finding, Remediation.
