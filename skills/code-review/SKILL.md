...
1. `capture-scope.mjs` then `build-scope-bundle.mjs` with identical flags (`--staged`, `--ref`, `--path`, `--focus`). No positional scope inference. `--path` values must be normalized repository-relative literal file paths.
