...
description: >-
  ...
  correctness or security hunt. Orchestrated runs (gauntlet) may scope to a
  workstream rather than a single commit.
---
...
4. Default model/thinking when unset: `cursor/grok-4.5` + `high` for every lens. Precedence: user override → this default → parent model. **Always name the model at the launch site** — do not rely on parent fallback.
5. **Prefix every subagent label** with its model — `(grok-4.5) Simplify lens: reuse`.
6. Only the **parent** edits the worktree. Reviewers report only.
7. Before any edit: re-check `git status` / the scoped diff. If the scope drifted since review, stop and report drift; do not apply stale suggestions.
8. Never commit unless the user explicitly asks.
9. When an orchestrator (e.g. `gauntlet` when present) scopes simplify to a **workstream** rather than a single commit, honor that scope. Pass through any **pinned decisions** from the plan so they are not "simplified away".
...
| Orchestrator-supplied workstream | Use the file list / range the parent named |

If the scoped diff is empty, say so and stop.

## Workflow

1. Capture scope; list files to the user; disclose four parallel high-reasoning simplify lenses when defaults apply. If pinned decisions were supplied, list them.
2. Launch four `px-simplifier` agents in one tool message (`run_in_background: true`), each prompt naming its lens and including the diff/file list, any pinned decisions, plus: `Report only. Behavior-preserving suggestions only. Do not edit. Return findings per your charter.` Labels: `(grok-4.5) Simplify lens: reuse|structure|efficiency|altitude`.
